...
// auth.api is a typed object generated by better-auth. We cast through `unknown`
// to a string-keyed intermediate so we can patch `getSession` for each test
// without losing type safety in the rest of the file.
type AuthApiMock = Record<string, unknown>;
...
describe('Test Files API Route (/api/test-files)', () => {
    ...
    beforeEach(() => {
        (auth.api as unknown as AuthApiMock).getSession = mock(() => Promise.resolve({
    ...
    test('GET /api/test-files returns 401 Unauthorized when session is missing', async () => {
        (auth.api as unknown as AuthApiMock).getSession = mock(() => Promise.resolve(null));
